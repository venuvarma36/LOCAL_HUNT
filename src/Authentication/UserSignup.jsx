...
export default function UserSignup() {
  ...
  const handleVerifyOtp = async () => {
    setMsg("");
    if (!otp) return setMsg("Please enter the OTP.");
    const normalized = normalizePhoneClient(phone);
    setBusy(true);
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    setBusy(true);
    try {
      const sres = await fetch("https://localhunt.onrender.com/signup/user", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          full_name: name,
  ...
  return (
    <main className="usersignup-main">
      <section className="usersignup-content left">
        <Lottie
      ...
      <section className="usersignup-content-right">
        ...
        <div className="otp-modal-overlay">
          <div className="otp-modal">
            ...
            {!isVerified && timer > 0 ? (
              ...
              !isVerified && (
                <p className="otp-expired-text">OTP expired. Please resend.</p>
              )
            )}
